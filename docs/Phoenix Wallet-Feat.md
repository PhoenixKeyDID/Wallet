...
- ⬜ **Fiat value (ADA in a local currency)** — deliberately **not** shipped. A price needs a source, and every candidate is a third party that would see a request from each user; this module currently sends nothing to any host except the chain indexer, and adding a second observer is a privacy decision rather than a feature decision. **Constraint in force until it is taken:** no price feed is wired, no fiat figure is displayed anywhere, and nothing in `src/` may add an outbound host — `check:urls` fails on an absolute URL outside a CODEOWNERS-gated file, so the fail-closed state is enforced rather than remembered.
...
- **No price or fiat-conversion service is contacted**, so no second third party sees a request from a user of this wallet. §7 records that as a decision with a constraint, not as a gap waiting to be filled quietly.
