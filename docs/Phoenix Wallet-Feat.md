...
|---|---|---|---|
| **Connect (CIP-30)** — Lace / Eternl / Typhon | yes, from the extension | yes, the extension signs | full |
| **Watch-only (`acct_xvk`)** | yes, derived client-side from the account public key | no, view only | yes |
| **Phoenix custody (by DID)** | yes, reads the script address + public balances | no, view only | view only |
| **Air-gap QR co-sign** | yes (reuses the watch-only key) | partial — offline device signs, returns a QR witness | partial — web scaffolded; enabled when the offline signer ships |
| **Local self-custody** | yes | yes, holds the key and signs Send / Staking / Governance through `WalletPort` (`keystore/port.ts`); reviewed only by the in-page confirm gate (§2.5), never by a popup outside the page | partial |
...
- dRep register/de-register use a hand-balanced fee path (the library does not reserve the dRep deposit); **vote and vote-delegation do not** — they carry no deposit and go through the library's own balancing, so an earlier version of this line naming "vote" described a path the floor never touched. A one-witness floor (101 bytes, minFeeA·101 = 4444 lovelace) is added on the hand-balanced path. It is **not** there because the estimator misses the certificate's key — measured, it does not: the library counts the dRep key, dedupes input credentials, and its estimate matches the assembled signed transaction to the byte (`__tests__/governance.fee.test.ts`). The floor covers the one term this code cannot see, the number of witnesses the **signer** chooses to attach. That surplus is spent, not refunded: on this path change is Σinputs − deposit − fee (§7).
...
**Nothing is signed that cannot be described.** `summariseTx` decodes the transaction body against an allow-list of known fields and refuses on anything it cannot render in words — an unknown body field, an inline datum, a reference script, a malformed policy id, and any collateral field (13, 16, 17). Collateral is refused rather than described because it is a second spend path: if a script in the transaction fails, the ledger takes the collateral inputs instead of the ordinary ones and pays back only the collateral return, wherever the transaction points it — so a page could put a large UTxO of this wallet up as collateral while the screen truthfully showed a small ordinary payment, and the key that signs the ordinary input signs for the collateral too. Describing it would mean knowing which collateral inputs are this wallet's, which is an open set; refusing needs no attribution, and costs nothing honest while this wallet does not sign smart-contract transactions (`getCollateral` is refused for the same reason). The approval screen then shows the refusal. It does not fall back to hex: hex on an approval dialog is not information, it is the appearance of information.
...
### 5.9 History — what this wallet has already done

Numbered after §5.8 rather than beside Send and Receive so that every `§5.8` reference elsewhere in this document keeps pointing where it did.

A wallet that can spend but cannot show what it spent sends the user to a block explorer, which hands a third party the link between their addresses and their browser — the exact link the rest of this module works to avoid. So the history is read here, and **no row links out**: the transaction id is shown in full and copyable, for anyone who wants to paste it into an explorer deliberately.

Every amount is signed **from the wallet's own side**, which is where a naive reading goes wrong three separate ways, all money-visible:

- **Change is not a payment.** An output returning to the wallet's own address is its own money coming back. Counting it turns "I paid 5 ADA" into "I paid 95".
- **A withdrawal is income that never appears as an input.** Staking rewards enter through the withdrawals field, so a net computed from inputs and outputs alone reports a transaction that emptied a reward account as having cost nothing — the same blind spot §7 records on the signing path, arrived at from the other direction.
- **The fee is already inside the difference.** Inputs cover the outputs *and* the fee, so subtracting it again double-counts every send.

A transaction has **several parties**, and every field in it belongs to one of them. Reading a shared field as though it were the wallet's own is the same class of error as counting change, and it is the one an outsider can trigger on purpose:

- **A withdrawal belongs to whoever owns the stake address inside it.** Anyone can pay this wallet 5 ADA in the same transaction that claims their own 100 ADA of staking rewards; summing every withdrawal and taking it off this wallet's total renders that as **"received −95 ADA"**, stated as confidently as any other row. Withdrawals are matched against this account's own reward address; a withdrawal with no owner named on it is refused outright. When the reward address is unknown — which a CIP-30 connection can leave it — a row carrying any withdrawal shows **no amount at all**, because both counting it and ignoring it are wrong in a different direction. In the two modes that ship, nothing reaches that state today: a CIP-30 connection withholds amounts one step earlier, on the incomplete address set, and a local account always has its own reward address. It is kept because the condition it protects against is *not knowing the reward address*, and which caller can find itself there is a property of the callers — which change — not of this rule.
- **The label answers "did *we* do this", not "did this happen near us".** A stranger minting an NFT, or registering their own stake key, in a transaction that merely takes a fee from this wallet would otherwise put "Minted a token" or "Staking changed" on a row where the wallet did neither. A mint counts only when the minted asset actually reached this wallet; a certificate counts only when it carries this account's stake address.
- **A counterparty is only a destination on a row that sent something.** The non-ours output addresses of a *receive* are the sender's own change addresses. Labelling those "To" tells the reader their money went somewhere it did not, and hands them an unrelated address in full, ready to copy into the next send. "Not ours" is read off the same address set the amounts are, so where that set is incomplete the "To" list is withheld along with them — otherwise the wallet's own change address is printed in full under **To**, on the one screen whose stated purpose is to be copied from.

- **The fee is only ours when we funded the transaction.** It comes out of the inputs, so on an ordinary receive the sender paid it and it is not inside the amount shown. Printing it with a minus sign there tells someone who has just received 5 ADA that they got 4.83, and sends them looking for the difference.

Four refusals, each in place of a number that would look plausible:

- A row carrying an address the module cannot read **throws** instead of being skipped. Skipping leaves every total short by exactly the part nobody could see.
- A row with **no timestamp or no block height** is refused rather than defaulted to zero, which would print `1/1/1970` and `Block 0` as facts.
- **A wallet that does not list all of its own addresses gets no amounts at all.** A CIP-30 extension answers with the addresses it chooses to admit to; every address it withheld reads here as a stranger's, so its own change becomes a payment. The screen states that the measurement is unavailable and why (`WalletPort.ownedIsComplete`, §7). Amounts appear for a locally-held account, which derived every address it has.
- A **confirmation count** is the distance to the chain tip at the moment of asking, not a property of the transaction — so the tip travels with the page, and when the tip sits behind an entry (two calls, a block minted between them) the screen shows nothing rather than `0`.

**One assumption on the indexer is stated rather than guarded, because there is nothing to guard against it with.** A transaction whose script phase failed is on the chain, keeps its collateral, and moves nothing else. Measured on mainnet 2026-09-08, `/tx_info` returns 27 top-level keys and **none of them says whether the transaction was valid** — so this module cannot tell such a row apart from an ordinary one, and would read it as a real transfer. The assumption is that the indexer does not list them. Writing it down is the point: an assumption nobody recorded is one nobody can check when it stops holding.

Koios omits inputs, withdrawals, asset lists and certificates from `/tx_info` unless each is asked for by flag, and — measured against the live indexer on 2026-09-06 — it returns those **keys** regardless, merely empty. A missing flag therefore does not fail; it answers "there were none", which is the shape of a wrong number rather than of an error. All four flags are named explicitly for that reason.

The listing asks for a **bounded page**, newest first, and the bound is not an optimisation. Koios is PostgREST and caps a response at 1000 rows; the module treats a truncated answer as an error rather than a short list (§5.8), so an unbounded listing would make the History tab fail *permanently* on exactly the accounts with the most to show. Measured on mainnet 2026-09-08: one real address answered `206 content-range=0-999/187078`, and a busy one answered **HTTP 504** to the unbounded question. A bound the caller set is not a truncation, and only the caller can tell the two apart, so it says which it asked.

Detail per row: transaction id, network fee, rewards collected, block number, confirmations, and — on a send — every counterparty address **in full**, since a shortened address is precisely what an address-swap survives.

**A refusal costs one row, not the page — and the row is still named.** `readTx` refuses rather than guesses, but reading the window as one unit meant the first refusal threw away the other twenty-four, and every retry hit the same newest-first window and failed identically. One cheap payment carrying a shape this module cannot read would have frozen a history indefinitely, with no explorer link to fall back on. Each row is now read on its own; the ones that failed are listed by transaction id above the others, because a list that is quietly short is the thing this whole feature argues against.

**Only the newest page is fetched, and the screen says so.** Without a way to widen the window, the newest N are all anyone can ever see: history past them is lost with nothing saying so, and about N cheap payments sent after a real one bury it for good — one address is enough, since every address of the account shares one window. The count is stated and can be widened up to a ceiling; the ceiling is the indexer's own 1000-row cap, past which a deliberate page and a truncated answer become indistinguishable again.

Rows sharing a block are ordered by transaction id as well as by height. Without a second sort key the cut at the page boundary lands differently on each read, so a row can appear, vanish and reappear with nothing having changed on the chain.

A read that fails leaves the previous list in place and says so on screen, with a retry. Emptying the list on a failed refresh would tell someone they have no transactions because the network blipped; a toast alone disappears in seconds and leaves a screen indistinguishable from a wallet that has never been used.

### 5.10 Multiple accounts from one recovery phrase

One phrase holds an unlimited number of BIP-44 accounts (`m/1852'/1815'/n'`), and they are separate wallets in every way that matters: separate addresses, separate balance, separate staking. The derivation was always here (`accountFromEntropy` takes the index, §4); what was missing was the way in.

The open wallet shows **which account it is on, next to the address**. Account 2's address looks exactly like account 0's, so without that line the only thing separating "my receive address" from "the wrong account's" is memory.

**Switching asks for the password again, by design.** The seed is erased the moment an account is derived (§2.1a), so nothing is left in memory to derive the next one from. Keeping the seed alive to make switching seamless would trade the module's central guarantee for the removal of one prompt. The account last opened is remembered per wallet (`StoredWallet.accountIndex`, optional so that records written before this default to account 0), so unlocking returns to where the money is rather than to an empty account 0 that reads as loss.

`WalletSession.unlock` wipes the previously open account before storing the new one, so a switch cannot leave the old account's keys in the heap — pinned by `keystore/__tests__/session.test.ts`. The account-switch form holds its own copy of the password, so the lock handler clears that field too; clearing only the unlock field would be a lock that locks one input and not the other.

### 5.11 Fiat value — the second outbound host

A price cannot be derived from the chain; it is what someone elsewhere is paying. Showing one therefore means asking a third party, and that party sees a request from each user. This is the only place in the module where a feature adds an observer, so the terms are written here rather than left to be inferred from a `fetch` call.

- **What the request carries:** the literal string `cardano` and a currency code. No address, no balance, no wallet identifier — the price of ADA does not depend on who is asking. What the other end learns is an IP and that somebody asked. That is strictly smaller than the indexer, which necessarily sees the addresses.
- **Off until asked for.** The line starts **off**, and no request is made until a reader picks a currency. Defaulting it on would mean that merely opening the wallet contacts a third party from the reader's address, at the same moment and from the same address as the indexer requests carrying their addresses, for a convenience nobody asked for. This is the same answer §3 gives to a question of the same shape — the network selector starts on the testnet, and reaching mainnet is something the user does on purpose. In the extension it matters more rather than less: the host permission is granted at install time, so there is no second prompt to notice.
- **Off is off.** `Off` is honoured before the request rather than in the display, and switching off also drops the cached rate — otherwise "off" would mean one component stopped drawing a figure the module was still holding. A switch that hid the number while still asking would be a switch that lies.
- **A rate is a reading, not a quote.** It was true at a moment, at one venue, and it is not what anyone will actually be paid. Every figure carries the time it was read, on screen next to it.
- **A price that cannot be read shows nothing** — never a stale figure, never `0.00`. A wallet printing zero because a request failed states a wrong fact about the reader's own money with nothing on screen to question it. Refusals are pinned by fixtures: no ADA row, a `null` rate, a string rate, zero, negative, and a figure large enough to be a different unit.
- **Only ADA gets a fiat line.** A native token's price is whatever its minter can find someone to quote, and a token can be minted with any name; putting a currency figure beside one would lend the wallet's own credibility to a number it cannot stand behind.
- The interface language decides which currency is offered *first* (đồng for `vi`, yen for `ja`, otherwise dollars) rather than being dollars for everyone — a reader converting in their head at a rate they have to guess has been handed arithmetic instead of an answer. It decides the order of the list, not whether to ask.
- The host literal is defined in `provider.ts`, not beside the code that uses it, because that file is what the gates read as the single answer to "who can this wallet talk to" (`check:urls`, `check:package`). A gate that reads two files is a gate with two places to forget.
...
- [partial] **Send + staking** verified end-to-end on preprod (send ADA, mint + send a native token, delegate to a stake pool). The governance dRep fee question is now **narrower than it was**: the fee arithmetic is settled offline — `__tests__/governance.fee.test.ts` assembles the real signed transaction (body plus a witness set of N dummy vkey witnesses) and checks the set fee against the node's own rule, minFeeA × size(signed) + minFeeB, rather than against the library's estimator, which is what the previous test compared and therefore could not settle. What still wants one preprod submit is the single term no offline check can reach: how many witnesses a real CIP-95 signer attaches to a DREP_REG. If it attaches exactly the required set, the one-witness floor can go and the fee becomes exact.
- [partial] **Air-gap QR co-sign** — web side scaffolded; turns on when the offline mobile signer ships. The QR transport contract (including the integrity binding in §6) is shared with the mobile signer.
- [done] **Local self-custody spends from this UI.** The adapter that was missing is `WalletPort`: `SendPanel`, `StakingPanel` and `GovernancePanel` take the port instead of a `Cip30Api`, `cip30Port()` wraps an extension and `localPort()` wraps an unlocked `Account`, and `LocalWalletPanel` mounts the same five tabs the Connect screen does. The port keeps `PhoenixNetwork` intact down to the signer instead of narrowing to a CIP-30 network id, which is what lets a preview account refuse a preprod transaction — CIP-30 calls both `0` and cannot tell them apart at all. `localPort` lives inside `src/lib/keystore/` so that `check:keystore-boundary` still holds: the panels import a finished port, never the keystore.
- [partial] **The local path's review is the page's own, and that is a real difference.** An extension signature is approved in a popup the page cannot draw over; a local signature is approved by the §2.5 confirm gate, in the same document as the wallet. Anyone who can execute script in this origin can therefore mis-draw the review it is checked against. The mitigations that exist are stated in §5 and are not equivalent to a separate window. A local wallet holding an amount worth attacking still belongs in hardware or an extension, and the UI says so.
- [partial] **Change and receive addresses are chosen from live balances, not address history.** `localPort` picks the first internal address holding no UTxO for change, and the first external one for receiving. That is not a BIP-44 gap scan: an address that received and then spent everything is indistinguishable here from one never used, so a busy wallet reuses addresses sooner than a full wallet would. The cost is privacy, not funds — and it shares a root cause with the gap-limit item below, since both need address *history* the balance query does not return.
- [broken] **Balance under-reports past 20 used addresses per chain.** Still true in the running app, and still marked *broken* for that reason. What now exists is everything except the wiring: `cardano/gapScan.ts` walks a chain in gap-limit batches until a whole batch holds nothing and returns how deep to derive, and `buildAccount`/`accountFromEntropy` take that depth — with a refusal to build *shallower* than `GAP_LIMIT`, since a narrower account is how funds stop being visible. Both are covered by `cardano/__tests__/gapScan.test.ts` and `keystore/__tests__/watchedSet.test.ts`, including that a deeper account is an extension of the shallow one rather than a different wallet. The probe asks `anyAddressFunded`, built on the balance endpoint already load-bearing here, not on an address-history endpoint whose row format would have to be assumed; that finds every address holding money, and deliberately does not claim the history scan that would also fix *address reuse* (the separate *partial* item above). **What is left is one decision, not one commit:** the scan needs a network round trip during unlock, and an `Account` does not retain the account private key it would need to extend itself afterwards — so either the seed entropy stays in memory across that round trip, or the account key is kept on the `Account` and scrubbed by `wipe()`. Both widen the key-material window in a wallet, differently, and that is the owner's call rather than an implementation detail. A gap *wider* than the limit is not crossed by this or by any BIP-44 wallet; the receive screen's own warning (§5.2) is what covers an address handed out past it.
- [partial] **CIP-30 injection (Phoenix as a provider to dApps)** — built in the extension (§5.8); **not yet loaded into a real Chrome**, so every claim about it rests on unit tests and a package check, not on a browser. Each constraint this item listed before it was built is now a specific mechanism, and each is tested: the origin comes from `sender.origin` and the request type has no origin field for a page to fill in; sub-frames are refused (`all_frames: false` in the manifest *and* a `frameId !== 0` refusal, because either alone is one edit from being undone); `summariseTx` refuses rather than guesses, and the screen shows the refusal instead of falling back to hex; the amount shown is the **net change to the wallet**, not the output total a change output inflates; reads sit behind a per-origin grant while `signTx`/`signData`/`submitTx` ask every time; and no password field exists anywhere in the injection path: the only unlock form a site's request can reach is on the `chrome-extension://` approval window (`LocalWalletPanel` has its own, which no website can open — §2.1a). One more mechanism was added after an adversarial pass found the screen and the signature disagreeing: **the keys a signature uses are derived from what the screen could describe, never from what the transaction asks for.** `required_signers` (field 14) lets a page name a key outright, and honouring that as written made the wallet an oracle — name the stake key and it signed the staking rewards away while a screen reading inputs and outputs reported a transaction that cost nothing. A named hash is now only *matched* against what the body already justified (an attributed input, a withdrawal from this wallet's own reward account, a certificate carrying its credential as `[0, hash]`); a hash the wallet holds but cannot explain is a refusal. In the same pass the summary learned to count withdrawals into the net amount, the screen learned to print a per-recipient amount and to say when it recognised only some of the inputs, and a second request arriving while one is on screen is refused rather than allowed to replace it under the reader's cursor. That window is a real window (`chrome.windows.create`), never the toolbar popup, which is destroyed when it loses focus while `LocalWalletPanel` locks on `visibilitychange`.
- [done] **Connecting is its own decision, and the approval window locks itself.** `enable` used to be answered the moment the window unlocked, which made typing a wallet password the act of granting a website access — the one human gesture in the flow, performed in a window a *web page* caused to appear. It is now a screen of its own with Allow and Reject, stating what the site gets (receiving addresses, balance, staking address) and what it does not (any ability to move funds; every payment is asked separately, every time). The window also re-locks after five idle minutes — `visibilitychange` is wrong for this one, since it is *meant* to sit behind the page it belongs to, so idle time is the measure that fits.
- [done] **A partial answer from the indexer is refused rather than shown.** Koios is PostgREST, which caps a response at 1000 rows without saying so in the status line: measured 2026-09-05, `/pool_list` answers HTTP 200 with `content-range: 0-999/*` and exactly 1000 rows. `koios()` now sends `Prefer: count=exact`, which makes the same request answer `206` with `content-range: 0-999/6163`, and throws `KoiosTruncatedError` when the range does not reach the total. Checking the status alone was never enough — `res.ok` is true for 206. A truncated UTxO list is a balance smaller than the truth and a coin selection blind to money the account holds, presented as fact.
- [done] **A wallet's address list is not a set to subtract from.** `WalletPort.ownedIsComplete` separates the two answers: a local account derived every address it has (closed), while a CIP-30 wallet answers with the addresses it chose to list — always a subset, and listing four is normal behaviour. The receive screen's "this address is not watched" warning now only fires against a closed set. Firing it on ordinary addresses of ordinary wallets is how a warning teaches people to dismiss it, and the case it exists for (an address outside the scanned range, where money really does become unreachable) arrives looking identical.
- [done] **Transaction history, with the amount told from the wallet's side** (§5.9). The module could spend and then had nothing to say about what it had spent, which is not cosmetic: it is the difference between checking what a dApp actually did after you approved it and going to a block explorer to find out. Three miscounts are pinned by tests rather than argued — change counted as a payment, a withdrawal read as money from nowhere, the fee subtracted twice — using the real figures of a mainnet transaction read from the live indexer, so the fixtures have the shape the indexer returns rather than the shape the module would prefer. An adversarial pass afterwards found a fourth, in the same family and reachable by an outsider on purpose: a transaction carries **several parties' withdrawals**, and counting them all rendered a 5 ADA payment received alongside a stranger's 100 ADA reward claim as "received −95 ADA". Attribution now runs off this account's own reward address, and a row it cannot attribute shows no amount. Each guard was re-broken to confirm the tests catch it — as a script rather than as a sentence, since a claim of the form "we checked" is exactly the kind that ages without anyone noticing: `scripts/mutants.mjs` reverts each guard in turn and requires the suite to go red, and it refuses to grade at all if the tree is red before it starts. Eleven guards, eleven reds. It is deliberately not wired into CI — a mutation run reverts source files on disk, and a job that does that on every push is a job one interrupted run leaves a repository broken by.
- [done] **Multiple accounts from one recovery phrase** (§5.10). `m/1852'/1815'/n'` was already derivable and had no way in, so every wallet was stuck on account 0. Switching re-derives from the vault and therefore asks for the password — the seed is gone by then, which is the property that makes it worth the prompt.
- [done] **Fiat value (ADA in a local currency)** — shipped, and it is the module's **second outbound host** and the first that is not a chain indexer (§5.11). That was a privacy decision rather than a feature one, and it was taken deliberately: a price cannot be derived from the chain, so showing one means asking someone. The exposure is bounded and stated — the request carries the string `cardano` and a currency code, nothing about the wallet, so what the other end learns is an IP and that somebody asked. It is **off until a reader turns it on**: shipping it on by default would have made first-open contact a third party for a convenience nobody asked for, which is the opposite of the answer this module gives everywhere else it faces that question. `off` is honoured before any request rather than only in the display, and it drops the cached rate too. The host literal lives in `provider.ts` so that the existing gates keep answering "who can this wallet talk to" from one file: `check:urls` requires an absolute URL to sit in a CODEOWNERS-gated file, and `check:package` requires every extension `host_permissions` entry to be a literal host backed by that same file (now 4, was 3). Both of those read text, and text is what an assembled URL is not — so the packaged extension now also declares `connect-src` in its content-security policy, which is enforced by the browser against whatever the code actually asks for at run time. The two lists have to agree exactly, in both directions, and `check:package` says so: a host in `connect-src` that no permission covers is a reach the gates never reviewed, and a permission with no `connect-src` entry is a request that will fail in the field and nowhere else. That check is worth nothing against a stale build, so it also refuses when anything under `src/` or `extension/` is newer than the packaged manifest.
- [not started] **Phoenix custody spend** — needs the controller-key path (air-gap / mobile), phase 2.
- [not started] **Multi-pool / multiple stake keys** — v1 delegates a single stake key; multi-stake-key management is a v2 item.
- [done] **Watch-only by single address** — shipped, and it is the **default** of the two watch paths. A single address links nothing and amplifies nothing (§2.2), so it is offered ahead of the `acct_xvk` one rather than beside it; the account-key path now carries an explicit warning that it exposes every address and the whole history. `parseWatchAddress` refuses a wrong-network address (which would otherwise return a truthful, terrifying 0 ADA) and a stake address (which holds no UTxO, so it would read as an empty wallet).
- [not started] **Randomised challenge position at confirm** — §2.5 always asks for the *last* four characters. Grinding a vanity address that matches one fixed window is cheap; grinding one that matches an unpredictable window ("type characters 18–21") is many times dearer, because the attacker must hit a position they cannot know in advance. Deliberately **not** shipped yet: it adds real friction to every send, and whether users tolerate reading into the middle of a bech32 string has to be tested with real people before it becomes the default. Until then the honest framing in §2.5 stands — this gate stops reflex ticks and mis-pastes, not a targeted grind.
- [not started] **Saved / known recipient addresses ("address book")** — a user-curated, labelled list of familiar destinations, so a send targets a chosen saved entry instead of a freshly pasted string an attacker can grind a look-alike of. Storage is **distributed on LampNet in a Strata** (the same model Smartsend uses); the Phoenix platform keeps none of the user's data on any server. A saved label is display-only: the full address is still shown and its tail retyped at confirm (§2.5), since a saved entry could itself have been poisoned when it was added. Strata record shape + client-side encryption to be specified.
...
- **The history screen queries the account's addresses together**, so Koios sees them in one request and can link them — which it already could, because the balance query has always sent the same list. The point of stating it is that the history screen is the one that removes a reason to visit a block explorer, and an explorer visit exposes the same link to a party with no reason to hold it and every reason to keep logs.
- **One price service is contacted, and only for the ADA rate** (§5.11). The request names no address and no amount, so what it exposes is an IP and the fact that somebody asked; it can be switched off, and `off` stops the request rather than only the display. This is the module's second outbound host and the first that is not a chain indexer — stated at this length because a fiat figure looks like a display detail and is in fact the one feature that added an observer.
