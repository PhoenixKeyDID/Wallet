...
Three refusals, each in place of a number that would look plausible:
...
Detail per row: transaction id, network fee, rewards collected, block number, confirmations, and every counterparty address **in full** — a shortened address is precisely what an address-swap survives.
...
- **Off is off.** The currency selector has an `Off` entry, and it is honoured before the request rather than in the display. A switch that hid the number while still asking would be a switch that lies.
...
- The default currency follows the interface language (đồng for `vi`, yen for `ja`, otherwise dollars) rather than being dollars for everyone — a reader converting in their head at a rate they have to guess has been handed arithmetic instead of an answer.
...
- ✅ **Transaction history, with the amount told from the wallet's side** (§5.9). The module could spend and then had nothing to say about what it had spent, which is not cosmetic: it is the difference between checking what a dApp actually did after you approved it and going to a block explorer to find out. Three miscounts are pinned by tests rather than argued — change counted as a payment, a withdrawal read as money from nowhere, the fee subtracted twice — using the real figures of a mainnet transaction read from the live indexer, so the fixtures have the shape the indexer returns rather than the shape the module would prefer. Each guard was re-broken to confirm the tests catch it (five mutations, five reds).
