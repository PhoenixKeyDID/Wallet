...
- **A counterparty is only a destination on a row that sent something.** The non-ours output addresses of a *receive* are the sender's own change addresses. Labelling those "To" tells the reader their money went somewhere it did not, and hands them an unrelated address in full, ready to copy into the next send.
